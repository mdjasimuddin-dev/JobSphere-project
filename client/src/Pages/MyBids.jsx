...
const MyBids = () => {
  ...
  const getData = async () => {
    const { data } = await axios.get(
      `${import.meta.env.VITE_APP_URL}/my-bids/${user?.email}`, {withCredentials : true}
    );
    setBids(data);
  };
  ...
  const handleStatus = async (id, currentStatus) => {
    const { data } = await axios.patch(
      `${import.meta.env.VITE_APP_URL}/bid-status/${id}`,
      { currentStatus },
    );
                            ...
                            className={`px-3 py-1 rounded-full
                                 text-xs ${
                                   bid.category === "Web Development"
                                     ? "text-blue-500 uppercase bg-blue-100/60"
                                     : bid.category === "Graphics Design"
                                     ? "text-emerald-500 uppercase bg-emerald-100/60"
                                     : bid.category === "Digital Marketing"
                                     ? "text-pink-500 uppercase bg-pink-100/60"
                                     : ""
                                 }`}
                          >
                            {bid.category}
